...
/**
 * PATCH /api/agencies/:id/toggle-payouts  (admin)
 */
exports.togglePayouts = async (req, res) => {
  try {
    const agency = await User.findOne({ _id: req.params.id, role: 'agency' });
    if (!agency) return res.status(404).json({ message: 'Agency not found' });
    agency.canViewPayouts = !agency.canViewPayouts;
    await agency.save();
    res.json({ agency: sanitizeAgency(agency) });
  } catch (err) {
    res.status(500).json({ message: err.message });
...
/**
 * PATCH /api/agencies/:id/toggle-default  (admin)
 * Only one agency can be default at a time.
 */
exports.toggleDefault = async (req, res) => {
  try {
    ...
    if (!agency.isDefaultAgency) {
      // Clear default from all others first
      await User.updateMany({ role: 'agency', _id: { $ne: agency._id } }, { isDefaultAgency: false });
    }
    agency.isDefaultAgency = !agency.isDefaultAgency;
    await agency.save();
    res.json({ agency: sanitizeAgency(agency) });
  } catch (err) {
    res.status(500).json({ message: err.message });
