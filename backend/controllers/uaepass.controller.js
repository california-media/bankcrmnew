...
exports.init = (req, res) => {
  ...
  try {
    ...
    if (referer) frontendUrl = new URL(referer).origin;
  } catch {}
...
exports.callback = async (req, res) => {
  ...
  // Validate CSRF state and extract frontendUrl captured at init time
  let frontendUrl = FRONTEND_URL;
  try {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return res.redirect(`${frontendUrl}/register?uaepass_error=token_failed`);
    }
    ...
    if (!infoRes.ok) {
      return res.redirect(`${frontendUrl}/register?uaepass_error=userinfo_failed`);
    }
    ...
    res.redirect(`${frontendUrl}/register?uaepass_error=server_error`);
  }
};
