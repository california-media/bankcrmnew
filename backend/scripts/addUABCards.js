...
const NEW_CARDS = [
  { name: 'UAB World Credit Card',     minSalary: 25000, cardType: 'legacy' },
  { name: 'UAB Platinum Credit Card',  minSalary: 10000, cardType: 'rewards_lifestyle' },
  { name: 'UAB Titanium Credit Card',  minSalary: 6000,  cardType: 'regular' },
];
...
async function run() {
  ...
  const bank = await Bank.findOneAndUpdate(
    { code: 'UAB' },
    { $setOnInsert: { name: 'United Arab Bank', code: 'UAB' } },
    { upsert: true, new: true }
  );
  console.log(`Bank: ${bank.name}`);

  const agency = await User.findOne({ email: 'agency3@gmail.com' });
  if (!agency) { console.error('agency3@gmail.com not found'); process.exit(1); }
