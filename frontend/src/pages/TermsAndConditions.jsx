...
const SECTIONS = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'sub', text: '6a. Card Fee — Free*(T&C)' },
      { type: 'p', text: 'Where a card product is marked "Free*(T&C)", the annual fee is waived for the first year only. From the second year onwards, charges will be applied in accordance with the issuing bank\'s policy. The client must be informed of this before submission.' },
    ],
  },
  {
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', padding: '40px 16px' }}>
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 40, paddingBottom: 32, borderBottom: '1px solid #e2e8f0' }}>
          <img src="/mysilah.svg" alt="Inizio Global" style={{ height: 48, width: 'auto', objectFit: 'contain', marginBottom: 24 }} />
          <div style={{ fontSize: 12, color: '#7C3AED', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 8 }}>Legal</div>
          ...
          <div style={{ marginTop: 28, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '20px 24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 24px' }}>
              {SECTIONS.map((s) => (
                <a key={s.num} href={`#section-${s.num}`} style={{ fontSize: 13, color: '#7C3AED', textDecoration: 'none' }}>
                  {s.num}. {s.title}
                </a>
              ))}
            </div>
