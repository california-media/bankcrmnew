...
function Agencies() {
  ...
  const onTogglePayouts = async (row) => {
    try {
      await api.patch(`/agencies/${row._id}/toggle-payouts`);
      message.success(row.canViewPayouts ? 'Payouts access revoked' : 'Payouts access granted');
      load();
    } catch (err) {
      message.error(err.response?.data?.message || 'Failed');
    }
  ...
  const onToggleDefault = async (row) => {
    try {
      await api.patch(`/agencies/${row._id}/toggle-default`);
      message.success(row.isDefaultAgency ? 'Default agency removed' : `${row.name || row.email} set as default agency`);
      load();
    } catch (err) {
      message.error(err.response?.data?.message || 'Failed');
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'canViewPayouts',
      width: 140,
      render: (v, row) => (
        <Tooltip title={v ? 'Click to revoke payouts access' : 'Click to grant payouts access'}>
          <Switch
            size="small"
            checked={!!v}
            onChange={() => onTogglePayouts(row)}
            checkedChildren="On"
    ...
    },
    {
      ...
      dataIndex: 'isDefaultAgency',
      width: 140,
      render: (v, row) => (
        <Tooltip title={v ? 'Remove as default (no-product leads)' : 'Set as default for leads without product'}>
          <Switch
            size="small"
            checked={!!v}
