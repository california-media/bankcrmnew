...
function AdminAgents() {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 110,
      align: 'right',
      render: (_, row) => <span style={{ fontWeight: 700, fontSize: 13, color: '#7C3AED' }}>{aed(row.stats?.paidCommission)}</span>,
    },
    {
          ...
          {agents.map((row) => (
            <Col key={row._id} xs={24} sm={12} lg={8}>
              <div style={{
                ...
                overflow: 'hidden',
                boxShadow: '0 2px 12px rgba(124,58,237,0.08), 0 1px 3px rgba(124,58,237,0.05)',
                transition: 'box-shadow 0.2s, transform 0.2s',
                cursor: 'pointer',
              }}
                onClick={() => navigate(`/admin/agents/${row._id}`)}
                onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 8px 28px rgba(124,58,237,0.18)'; e.currentTarget.style.transform = 'translateY(-1.5px)'; e.currentTarget.style.borderColor = '#c4b5fd'; }}
                onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 2px 12px rgba(124,58,237,0.08), 0 1px 3px rgba(124,58,237,0.05)'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = '#ede9fe'; }}
              >
                <div style={{ padding: '20px 20px 16px', display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
                  ...
                  {row.agency?.name && (
                    <div style={{ display: 'inline-flex' }}>
                      <span style={{ fontSize: 12, color: '#7C3AED', background: '#f3e8ff', border: '1px solid #e0e0ff', borderRadius: 20, padding: '2px 12px', fontWeight: 500 }}>
                        {row.agency.name}
                      </span>
                    </div>
                  )}
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#7C3AED', marginLeft: 8 }}>{aed(row.stats?.paidCommission)}</span>
                  </div>
