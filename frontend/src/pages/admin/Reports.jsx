...
function AdminReports() {
  ...
  const perfData = useMemo(() => {
    const map = new Map();
    filtered.forEach(l => {
      const key = `${l.agency?._id}|${l.bank?._id}|${l.productType}|${l.agent?._id}`;
      if (!map.has(key)) {
        map.set(key, {
          key,
    ...
    return [...map.values()].sort((a, b) => a.agency.localeCompare(b.agency) || a.bank.localeCompare(b.bank));
  }, [filtered]);
  ...
  const columns = [
    ...
    { title: 'Agency', render: (_, r) => r.agency?.name || r.agency?.email || '—' },
          ...
          <Card title="Leads Over Time (12 months)" size="small" style={{ borderRadius: 12 }}>
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={byMonth} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                ...
                <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                <Tooltip />
                <Line type="monotone" dataKey="count" stroke="#7C3AED" strokeWidth={2} dot={{ r: 3 }} name="Leads" />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </Col>
            ...
            summary={rows => {
              ...
              return (
                <Table.Summary.Row style={{ fontWeight: 700, background: '#f8fafc' }}>
                  ...
                  <Table.Summary.Cell index={3}>{tot('newLead')}</Table.Summary.Cell>
                  <Table.Summary.Cell index={4}>{tot('approved')}</Table.Summary.Cell>
                  <Table.Summary.Cell index={5}>{tot('rejected')}</Table.Summary.Cell>
                  <Table.Summary.Cell index={6}>{tot('disbursed')}</Table.Summary.Cell>
