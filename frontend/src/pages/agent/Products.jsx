        ...
        <div style={{ background: '#f8fafc', borderRadius: 8, padding: '12px 14px' }}>
          ...
          {product.productType === 'credit_card' && firstBracket.feeType && (
            <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Tag color={feeTypeColors(firstBracket.feeType).tag} style={{ fontSize: 10, margin: 0 }}>
                {feeTypeLabel(firstBracket.feeType)}
              </Tag>
              {hasImage && (
                <img
    ...
    <>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={12} lg={6}>
          <StatCard icon={<FundOutlined />} iconColor="#7C3AED" label="Live Products" value={stats.total} borderColor="#7C3AED" bg="#f3e8ff" />
        </Col>
        <Col xs={24} sm={12} lg={6}>
      ...
      <div className="leads-filter-bar" style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {[
            { value: 'all',         label: 'All',          color: '#7C3AED', bg: '#f3e8ff', border: '#7C3AED' },
