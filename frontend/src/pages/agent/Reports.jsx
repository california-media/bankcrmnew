...
function AgentReports() {
  ...
  const perfData = useMemo(() => {
    const map = new Map();
    filtered.forEach(l => {
      const key = `${l.bank?._id}|${l.productType}`;
      if (!map.has(key)) {
        map.set(key, {
          key,
          bank: l.bank?.name || '—',
            ...
            summary={rows => {
              ...
              return (
                <Table.Summary.Row style={{ fontWeight: 700, background: '#f8fafc' }}>
                  ...
                  <Table.Summary.Cell index={2} align="center">{tot('newLead')}</Table.Summary.Cell>
                  <Table.Summary.Cell index={3} align="center">{tot('approved')}</Table.Summary.Cell>
                  <Table.Summary.Cell index={4} align="center">{tot('rejected')}</Table.Summary.Cell>
                  <Table.Summary.Cell index={5} align="center">{tot('disbursed')}</Table.Summary.Cell>
                </Table.Summary.Row>
              );
            }}
            columns={[
