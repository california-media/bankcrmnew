...
6a. Card Fee — Free*(T&C)
   Where a card product is marked "Free*(T&C)", the annual fee is waived for the first year only. From the second year onwards, charges will be applied in accordance with the issuing bank's policy. The client must be informed of this before submission.
...
function SubmitLead() {
  ...
  const onTermsConfirm = async () => {
    ...
    try {
      ...
      payload.customerSalary = values.salary;
      if (values.salaryBracket != null) payload.salaryBracket = values.salaryBracket;
    ...
    <>
      ...
      <Form form={form} layout="vertical">
        <Row gutter={12} align="stretch">
          {/* LEFT — Client */}
          <Col xs={24} lg={14}>
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e8ecf4', borderTop: '3px solid #7C3AED', boxShadow: '0 4px 18px rgba(124,58,237,0.08)', padding: '14px 18px', height: '100%' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, paddingBottom: 10, borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: '#f3e8ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7C3AED" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                </div>
              ...
              <Row gutter={[10, 0]}>
                <Col xs={24} sm={12}>
                  <Form.Item name="customerName" label={<span style={{ fontWeight: 600, fontSize: 12, color: '#374151' }}>Full Name (as per Emirates ID) <span style={{ color: '#ef4444' }}>*</span></span>} rules={[{ required: true, message: 'Name required' }]} style={{ marginBottom: 10 }}>
                ...
                <Col xs={24} sm={12}>
                  <Form.Item name="email" label={<span style={{ fontWeight: 600, fontSize: 12, color: '#374151' }}>Email <span style={{ color: '#ef4444' }}>*</span></span>} rules={[{ required: true, message: 'Email required' }, { type: 'email', message: 'Invalid email' }]} style={{ marginBottom: 10 }}>
                    <Input size="middle" placeholder="client@email.com" style={{ borderRadius: 8 }} />
                  </Form.Item>
                </Col>
                <Col xs={24} sm={12}>
                  <Form.Item name="salary" label={<span style={{ fontWeight: 600, fontSize: 12, color: '#374151' }}>Monthly Salary (AED, min 5,000) <span style={{ color: '#ef4444' }}>*</span></span>} rules={[{ required: true, message: 'Salary required' }, { type: 'number', min: 5000, message: 'Minimum salary is AED 5,000' }]} style={{ marginBottom: 10 }}>
                    <InputNumber size="middle" min={0} step={500} style={{ width: '100%', borderRadius: 8 }} placeholder="e.g. 8000" formatter={v => v ? `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',') : ''} parser={v => v.replace(/,/g, '')} />
                  </Form.Item>
                </Col>
                <Col xs={24} sm={12}>
                ...
                <Col xs={24} sm={12}>
                  <Form.Item name="city" label={<span style={{ fontWeight: 600, fontSize: 12, color: '#374151' }}>City</span>} style={{ marginBottom: 10 }}>
                    <Select size="middle" allowClear placeholder="Select city" options={CITIES} />
                  </Form.Item>
                </Col>
                <Col xs={24} sm={12}>
                ...
                <Col xs={24} sm={12}>
                  <Form.Item name="yearsOfExperience" label={<span style={{ fontWeight: 600, fontSize: 12, color: '#374151' }}>Length of Service (yrs)</span>} style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, paddingBottom: 8, borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: productType === 'credit_card' ? '#f3e8ff' : '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}>
                  {productType === 'credit_card'
                    ? <CreditCardOutlined style={{ color: '#7C3AED', fontSize: 14 }} />
                      ...
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {selectedBracket?.feeType && (
                            <span style={{ display: 'inline-block', marginTop: 3, fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 999, background: feeTypeColors(selectedBracket.feeType).bg, color: feeTypeColors(selectedBracket.feeType).text, border: `1px solid ${feeTypeColors(selectedBracket.feeType).border}` }}>
        ...
        <div style={{ marginTop: 10, background: '#fff', borderRadius: 12, border: '1px solid #e8ecf4', padding: '12px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 10px rgba(15,23,42,0.06)' }}>
          ...
          <Space>
            ...
            <Button
              ...
              style={{ borderRadius: 8, background: 'linear-gradient(90deg, #7C3AED, #0EA5E9)', border: 'none', fontWeight: 700, paddingLeft: 20, paddingRight: 20, height: 38, boxShadow: '0 4px 14px rgba(124,58,237,0.40)' }}
            >
              Submit Lead
            </Button>
          </Space>
