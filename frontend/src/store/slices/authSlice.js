...
export const registerAgent = createAsyncThunk('auth/registerAgent', async (payload, { rejectWithValue }) => {
  try {
    ...
    return data; // { message: '...' } — no token, awaiting email verification
  } catch (err) {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    [login, setPassword].forEach((thunk) => {
      builder
    ...
    builder
      .addCase(registerAgent.pending, (state) => { state.status = 'loading'; state.error = null; })
